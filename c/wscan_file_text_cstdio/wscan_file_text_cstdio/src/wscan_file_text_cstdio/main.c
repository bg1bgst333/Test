...
int wscan_file_text_cstdio(const char *path, char *text); /* 関数wscan_file_text_cstdioの宣言. */
...
int main(void){
  ...
  wscan_file_text_cstdio("test1.txt", buf0); /* wscan_file_text_cstdioで"test1.txt"を読み込み. */
  ...
  wscan_file_text_cstdio("test1.txt", buf1); /* wscan_file_text_cstdioで"test1.txt"を読み込み. */
  ...
  wscan_file_text_cstdio("test2.txt", buf2); /* wscan_file_text_cstdioで"test2.txt"を読み込み. */
...
int wscan_file_text_cstdio(const char *path, char *text){
  ...
  if (fp != NULL){ /* fpがNULLでない時. */
    ...
    /* ファイルからテキストを読み込む. */
    ret = fwscanf(fp, L"%s", text); /* fwscanfでfpから入力しtextに格納, 戻り値はretに格納. */

...
int wscan_file_wtext_ccs_cstdio(const char* path, wchar_t* wtext, const char* ccs);	/* 関数wscan_file_wtext_ccs_cstdioの宣言. */
...
int main(void) {
	...
	size_t file_size_4;	/* size_t型変数file_size_4. */
	...
	wscan_file_wtext_ccs_cstdio("test1.txt", buf1, NULL);	/* wscan_file_wtext_ccs_cstdioでNULLの"test1.txt"を読み込み. */
	...
	wscan_file_wtext_ccs_cstdio("test2.txt", buf2, "");	/* wscan_file_wtext_ccs_cstdioで""の"test2.txt"を読み込み. */
	...
	wscan_file_wtext_ccs_cstdio("test3.txt", buf3, "UNICODE");	/* wscan_file_wtext_ccs_cstdioで"UNICODE"の"test3.txt"を読み込み. */
	...
	wscan_file_wtext_ccs_cstdio("test4.txt", buf4, "UTF-8");	/* wscan_file_wtext_ccs_cstdioで"UTF-8"の"test4.txt"を読み込み. */
...
int wscan_file_wtext_ccs_cstdio(const char* path, wchar_t* wtext, const char* ccs) {
	...
	if (fp != NULL) {	/* fpがNULLでない時. */
		...
		ret = fwscanf(fp, L"%ls", wtext);	/* fwscanfでfpから入力しwtextに格納, 戻り値はretに格納. */

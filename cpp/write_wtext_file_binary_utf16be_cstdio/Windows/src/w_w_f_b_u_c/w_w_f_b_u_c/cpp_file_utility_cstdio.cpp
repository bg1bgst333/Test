...
// 指定のワイド文字列をUTF-16BEでバイナリモードで書き込む.
int write_wtext_file_binary_utf16be_cstdio(const char* path, const wchar_t* wtext) {
	...
	// ファイルに書き込む.
	int ret = write_wtext_file_binary_cstdio(path, (wchar_t*)converted);
	delete[] converted;
	return ret;

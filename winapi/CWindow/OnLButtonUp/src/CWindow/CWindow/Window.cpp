...
void CWindow::OnLButtonDown(UINT nFlags, POINT pt) {
	...
	// 各パラメータを出力.
	TCHAR tszText[256] = { 0 };
	_stprintf(tszText, _T("OnLButtonDown nFlags = %lu, pt.x = %d, pt.y = %d\n"), nFlags, pt.x, pt.y);
	MessageBox(NULL, tszText, _T("CWindow"), MB_OK);
...
void CWindow::OnLButtonUp(UINT nFlags, POINT pt) {
	...
	// 各パラメータを出力.
	TCHAR tszText[256] = { 0 };
	_stprintf(tszText, _T("OnLButtonUp nFlags = %lu, pt.x = %d, pt.y = %d\n"), nFlags, pt.x, pt.y);
	MessageBox(NULL, tszText, _T("CWindow"), MB_OK);

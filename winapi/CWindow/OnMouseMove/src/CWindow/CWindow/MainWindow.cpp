...
int CMainWindow::OnCreate(HWND hwnd, LPCREATESTRUCT lpCreateStruct){
	...
	m_pScreen->Create(_T("CWindow"), WS_BORDER, 0, 0, 400, 300, hwnd, (HMENU)(WM_APP + 1), lpCreateStruct->hInstance);	// Createでスクリーンコントロールのウィンドウ作成.
...
int CMainWindow::OnFileOpen(WPARAM wParam, LPARAM lParam){
	...
	if (ret == IDOK){
		...
		m_pScreen = new CScreen();	// newでCScreenオブジェクトを作成し, ポインタm_pScreenに格納.
		m_pScreen->Create(_T("CWindow"), WS_BORDER, 0, 0, 400, 300, m_hWnd, (HMENU)(WM_APP + 1), m_hInstance);	// Createでスクリーンコントロールのウィンドウ作成.

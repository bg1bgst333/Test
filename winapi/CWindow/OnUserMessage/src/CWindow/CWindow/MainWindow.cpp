...
int CMainWindow::OnCreate(HWND hwnd, LPCREATESTRUCT lpCreateStruct) {
	...
	if (m_pMainMenu == NULL) {	// メニューハンドルが無い場合は, m_pMainMenuがNULLになる.
		...
		if (bRet) {
			...
			m_pWindow0 = pCustomEdit;
			pCustomEdit->Create(_T("Item0-CustomEdit"), WS_CHILD | WS_VISIBLE | WS_HSCROLL | WS_VSCROLL | ES_MULTILINE | ES_WANTRETURN | ES_AUTOHSCROLL | ES_AUTOVSCROLL, 0, 0, 160, 160, pItem0->m_hWnd, (HMENU)WM_APP + 200, m_hInstance);
		}
	}
	return iRet;	// iRetを返す.
...
int CMainWindow::OnFileOpen(WPARAM wParam, LPARAM lParam) {
	...
	if (ret == IDOK) {
		MessageBox(m_hWnd, dlg.GetOFN().lpstrFile, _T("CWindow"), MB_OK);
	}

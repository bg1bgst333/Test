...
int CMainWindow::OnCreate(HWND hwnd, LPCREATESTRUCT lpCreateStruct){
	...
	m_pScreen->Create(_T("CDIBSection"), WS_BORDER, 0, 0, 400, 300, hwnd, (HMENU)(WM_APP + 1), lpCreateStruct->hInstance);	// Createでスクリーンコントロールのウィンドウ作成.
...
int CMainWindow::OnClose(){
	...
	int iRet = MessageBox(m_hWnd, _T("Close CMainWindow OK?"), _T("CDIBSection"), MB_OKCANCEL);	// MessageBoxで"Close CMainWindow OK?"と表示し, 戻り値をiRetに格納.
...
int CMainWindow::OnFileSaveAs(WPARAM wParam, LPARAM lParam){
	...
	if (ret == IDOK){
		if (m_pScreen != NULL){
			if (m_pScreen->m_pDIBSection != NULL){
				if (m_pScreen->m_pCanvas != NULL){
					m_pScreen->m_pDIBSection->SaveBitmap(m_pScreen->m_pCanvas->m_hWnd, dlg.GetOFN().lpstrFile);	// SaveBitmapで保存.
				}
			}
		}
	}

...
int CMainWindow::OnCreate(HWND hwnd, LPCREATESTRUCT lpCreateStruct){
	...
	m_pStatic = new CStaticCore();	// newでCStaticCoreオブジェクトを作成し, ポインタm_pStaticに格納.

	// スタティックコアコントロールのウィンドウ作成.
	RECT rc;	// RECT構造体rc.
	rc.left = 50;		// 左50
	rc.right = 150;		// 右150
	rc.top = 50;		// 上50
	rc.bottom = 150;	// 下150
	m_pStatic->Create(_T(""), WS_BORDER | SS_BITMAP, rc, hwnd, (HMENU)(WM_APP + 1), lpCreateStruct->hInstance);	// Createでスタティックコアコントロールのウィンドウ作成.
	...
	m_pStatic->SetBitmap(*m_pBitmap);	// CStaticCore::SetBitmapで*m_pBitmapをセット.

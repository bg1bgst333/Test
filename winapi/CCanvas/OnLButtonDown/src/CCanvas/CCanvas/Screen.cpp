...
int CScreen::OnCreate(HWND hwnd, LPCREATESTRUCT lpCreateStruct) {
	...
	m_pCanvas->Create(_T("CCanvas"), 0, 50, 50, 400, 300, hwnd, (HMENU)(WM_APP + 2), lpCreateStruct->hInstance);	// Createでキャンバスコントロールのウィンドウ作成.

	// DIBセクションの作成.
	m_pDIBSection = new CDIBSection();
	m_pDIBSection->CreateDIBSection(m_pCanvas->m_hWnd, 320, 240);

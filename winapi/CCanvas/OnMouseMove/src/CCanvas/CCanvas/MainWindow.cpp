...
int CMainWindow::OnCreate(HWND hwnd, LPCREATESTRUCT lpCreateStruct){
	...
	m_pScreen->Create(_T("CCanvas"), WS_BORDER, 0, 0, 400, 300, hwnd, (HMENU)(WM_APP + 1), lpCreateStruct->hInstance);	// Createでスクリーンコントロールのウィンドウ作成.
...
int CMainWindow::OnClose(){
	...
	int iRet = MessageBox(m_hWnd, _T("Close CMainWindow OK?"), _T("CCanvas"), MB_OKCANCEL);	// MessageBoxで"Close CMainWindow OK?"と表示し, 戻り値をiRetに格納.
...
int CMainWindow::OnFileOpen(WPARAM wParam, LPARAM lParam){
	...
	if (ret == IDOK){
		RGBQUAD prevColor = {0};
		if (m_pScreen != NULL){
			prevColor = m_pScreen->m_pCanvas->m_rgbqCurrentColor;
			delete m_pScreen;
			m_pScreen = NULL;
		}
		m_pScreen = new CScreen();	// newでCScreenオブジェクトを作成し, ポインタm_pScreenに格納.
		m_pScreen->Create(_T("CCanvas"), WS_BORDER, 0, 0, 400, 300, m_hWnd, (HMENU)(WM_APP + 1), m_hInstance);	// Createでスクリーンコントロールのウィンドウ作成.
		...
		m_pScreen->m_pCanvas->SetCurrentColor(prevColor.rgbRed, prevColor.rgbGreen, prevColor.rgbBlue);
	}

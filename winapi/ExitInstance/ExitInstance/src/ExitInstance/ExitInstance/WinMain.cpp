...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd) {
	...
	if (!InitInstance(hInstance, lpCmdLine, nShowCmd)) {	// InitInstanceがFALSEの場合.
		...
		// 異常終了.
		ExitInstance();	// ExitInstanceで終了処理を実行.
		return -1;	// returnで-1を返して異常終了.

	}

	// メッセージループ処理.
	return Run();	// Runの戻り値をそのまま返す.
...
BOOL InitInstance(HINSTANCE hInstance, LPTSTR lpCmdLine, int nShowCmd) {
	...
	wc.lpszClassName = _T("ExitInstance");	// ウィンドウクラス名はとりあえず"ExitInstance"とする.
	...
	// ウィンドウの作成.
	hWnd = CreateWindow(wc.lpszClassName, _T("ExitInstance"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowでウィンドウクラス名が"ExitInstance"なウィンドウ"ExitInstance"を作成.

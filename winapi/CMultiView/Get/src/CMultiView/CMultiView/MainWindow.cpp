...
int CMainWindow::OnCreate(HWND hwnd, LPCREATESTRUCT lpCreateStruct) {
	...
	if (m_pMainMenu == NULL) {	// メニューハンドルが無い場合は, m_pMainMenuがNULLになる.
		...
		if (bRet) {
			...
			m_pMultiView->Add(_T("Item3"), 380, 20, 160, 120, m_hInstance);
			// マルチビューアイテムの取得.
			CMultiViewItem* pItem0 = m_pMultiView->Get(0);
			// マルチビューアイテム内にウィンドウを配置.
			m_pWindow0 = new CWindow();
			m_pWindow0->Create(WC_EDIT, _T("Item0-Edit"), WS_CHILD | WS_VISIBLE | WS_HSCROLL | WS_VSCROLL | ES_MULTILINE | ES_WANTRETURN | ES_AUTOHSCROLL | ES_AUTOVSCROLL, 10, 10, 80, 80, pItem0->m_hWnd, (HMENU)WM_APP + 200, m_hInstance);
		}
	}
	return iRet;	// iRetを返す.
...
int CMainWindow::OnClose() {
	...
	int iRet = MessageBox(m_hWnd, _T("Close CMainWindow OK?"), _T("CMultiView"), MB_OKCANCEL);	// MessageBoxで"Close CMainWindow OK?"と表示し, 戻り値をiRetに格納.
...
int CMainWindow::OnFileOpen(WPARAM wParam, LPARAM lParam) {
	...
	if (ret == IDOK) {
		MessageBox(m_hWnd, dlg.GetOFN().lpstrFile, _T("CMultiView"), MB_OK);
	}

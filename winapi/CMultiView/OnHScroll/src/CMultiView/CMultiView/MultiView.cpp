...
void CMultiView::OnSize(UINT nType, int cx, int cy){
	...
	GetWindowRect(m_pMultiViewItemsPanel->m_hWnd, &mvitemsPanelRC);
	...
	m_ScrollInfo.nMin = 0;
	m_ScrollInfo.nMax = mvitemsPanelRC.right - mvitemsPanelRC.left;
	...
	m_ScrollInfo.nMin = 0;
	m_ScrollInfo.nMax = mvitemsPanelRC.bottom - mvitemsPanelRC.top;
...
void CMultiView::OnHScroll(UINT nSBCode, UINT nPos){
	...
	::GetWindowRect(m_pMultiViewItemsPanel->m_hWnd, &rcItemsPanelRC);

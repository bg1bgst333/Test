...
int CMainWindow::OnCreate(HWND hwnd, LPCREATESTRUCT lpCreateStruct){
	...
	if (m_pMainMenu == NULL){	// メニューハンドルが無い場合は, m_pMainMenuがNULLになる.
		...
		if (bRet){
			...
			AddCommandHandler(ID_ITEM_3_1, 0, (int(CWindow::*)(WPARAM, LPARAM))&CMainWindow::OnItem3_1);
			...
			// Item1-1にラジオチェックを付ける.
			pSubMenu1->CheckMenuRadioItem(ID_ITEM_BOM_NONE, ID_ITEM_BOM_UTF8, ID_ITEM_BOM_NONE, MF_BYCOMMAND);
			m_pTextFile->m_Bom = CTextFile::BOM_NONE;	// BOMはNONEとする.
			// Item1-3にラジオチェックを付ける.
			pSubMenu1->CheckMenuRadioItem(ID_ITEM_ENCODE_SHIFTJIS, ID_ITEM_ENCODE_JIS, ID_ITEM_ENCODE_SHIFTJIS, MF_BYCOMMAND);
			m_pTextFile->m_Encoding = CTextFile::ENCODING_SHIFT_JIS;	// Shift_JISとする.
			// Item1-5にラジオチェックを付ける.
			pSubMenu1->CheckMenuRadioItem(ID_ITEM_NEW_LINE_CRLF, ID_ITEM_NEW_LINE_CR, ID_ITEM_NEW_LINE_CRLF, MF_BYCOMMAND);
			m_pTextFile->m_NewLine = CTextFile::NEW_LINE_CRLF;	// CRLFとする.
			// Item3-1にラジオチェックを付けない.
			pSubMenu3->CheckMenuItem(ID_ITEM_3_1, MF_BYCOMMAND | MF_UNCHECKED);
...
void CMainWindow::OnDestroy(){
	...
	DeleteCommandHandler(ID_ITEM_3_1, 0);
...
// Item3-1が選択された時.
int CMainWindow::OnItem3_1(WPARAM wParam, LPARAM lParam){

	// 文字コード推測オフならオン.
	if (!m_pTextFile->m_bDetectEnc){

		// Item3-1にラジオチェックを付ける.
		CMenu *pSubMenu2 = m_pMainMenu->GetSubMenu(2);
		pSubMenu2->CheckMenuItem(ID_ITEM_3_1, MF_BYCOMMAND | MF_CHECKED);
		...
		// Item3-1にラジオチェックを外す.
		CMenu *pSubMenu2 = m_pMainMenu->GetSubMenu(2);
		pSubMenu2->CheckMenuItem(ID_ITEM_3_1, MF_BYCOMMAND | MF_UNCHECKED);
		m_pTextFile->m_bDetectEnc = FALSE;

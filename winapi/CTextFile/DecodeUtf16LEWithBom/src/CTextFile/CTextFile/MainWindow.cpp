...
int CMainWindow::OnCreate(HWND hwnd, LPCREATESTRUCT lpCreateStruct){
	...
	if (m_pMainMenu == NULL){	// メニューハンドルが無い場合は, m_pMainMenuがNULLになる.
		...
		if (bRet){
			...
			pSubMenu1->AppendMenu(MF_STRING, ID_ITEM_1_5, _T("Item1-5(&E)"));
			pSubMenu1->AppendMenu(MF_STRING, ID_ITEM_1_6, _T("Item1-6(&F)"));
			pSubMenu1->AppendMenu(MF_STRING, ID_ITEM_1_7, _T("Item1-7(&G)"));
			// メインメニューに追加.
			m_pMainMenu->AppendMenu(MF_POPUP, (UINT_PTR)pSubMenu1->m_hMenu, _T("Item1"));
			// サブメニュー2の作成.
			CMenu *pSubMenu2 = new CMenu();
			pSubMenu2->CreatePopupMenu();
			pSubMenu2->AppendMenu(MF_STRING, ID_ITEM_2_1, _T("Item2-1(&H)"));
			pSubMenu2->AppendMenu(MF_STRING, ID_ITEM_2_2, _T("Item2-2(&I)"));
			pSubMenu2->AppendMenu(MF_STRING, ID_ITEM_2_3, _T("Item2-3(&J)"));
			pSubMenu2->AppendMenu(MF_STRING, ID_ITEM_2_4, _T("Item2-4(&K)"));
			// メインメニューに追加.
			...
			AddCommandHandler(ID_ITEM_1_5, 0, (int(CWindow::*)(WPARAM, LPARAM))&CMainWindow::OnItem1_5);	// AddCommandHandlerでID_ITEM_1_5に対するハンドラCMainWindow::OnItem1_5を登録.
			AddCommandHandler(ID_ITEM_1_6, 0, (int(CWindow::*)(WPARAM, LPARAM))&CMainWindow::OnItem1_6);	// AddCommandHandlerでID_ITEM_1_6に対するハンドラCMainWindow::OnItem1_6を登録.
			AddCommandHandler(ID_ITEM_1_7, 0, (int(CWindow::*)(WPARAM, LPARAM))&CMainWindow::OnItem1_7);	// AddCommandHandlerでID_ITEM_1_7に対するハンドラCMainWindow::OnItem1_7を登録.
			...
			m_pTextFile->m_Bom = CTextFile::BOM_NONE;	// BOMはNONEとする.
			// Item1-3にラジオチェックを付ける.
			pSubMenu0->CheckMenuRadioItem(ID_ITEM_1_3, ID_ITEM_1_4, ID_ITEM_1_3, MF_BYCOMMAND);
			m_pTextFile->m_Encoding = CTextFile::ENCODING_SHIFT_JIS;	// Shift_JISとする.
			// Item1-5にラジオチェックを付ける.
			pSubMenu0->CheckMenuRadioItem(ID_ITEM_1_5, ID_ITEM_1_7, ID_ITEM_1_5, MF_BYCOMMAND);
...
void CMainWindow::OnDestroy(){
	...
	DeleteCommandHandler(ID_ITEM_1_5, 0);	// DeleteCommandHandlerでID_ITEM_1_5のハンドラを削除.
	DeleteCommandHandler(ID_ITEM_1_6, 0);	// DeleteCommandHandlerでID_ITEM_1_6のハンドラを削除.
	DeleteCommandHandler(ID_ITEM_1_7, 0);	// DeleteCommandHandlerでID_ITEM_1_7のハンドラを削除.
...
int CMainWindow::OnItem1_2(WPARAM wParam, LPARAM lParam){
	...
	pSubMenu0->CheckMenuRadioItem(ID_ITEM_1_3, ID_ITEM_1_4, ID_ITEM_1_4, MF_BYCOMMAND);
...
// Item1-5が選択された時.
int CMainWindow::OnItem1_5(WPARAM wParam, LPARAM lParam){
...
// Item1-6が選択された時.
int CMainWindow::OnItem1_6(WPARAM wParam, LPARAM lParam){

	// Item1-6にラジオチェックを付ける.
	CMenu *pSubMenu0 = m_pMainMenu->GetSubMenu(0);
	pSubMenu0->CheckMenuRadioItem(ID_ITEM_1_5, ID_ITEM_1_7, ID_ITEM_1_6, MF_BYCOMMAND);
...
// Item1-7が選択された時.
int CMainWindow::OnItem1_7(WPARAM wParam, LPARAM lParam){

	// Item1-7にラジオチェックを付ける.
	CMenu *pSubMenu0 = m_pMainMenu->GetSubMenu(0);
	pSubMenu0->CheckMenuRadioItem(ID_ITEM_1_5, ID_ITEM_1_7, ID_ITEM_1_7, MF_BYCOMMAND);

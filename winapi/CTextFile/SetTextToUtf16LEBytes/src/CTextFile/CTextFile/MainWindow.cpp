...
int CMainWindow::OnItem1_1(WPARAM wParam, LPARAM lParam){
	...
	pSubMenu0->CheckMenuRadioItem(ID_ITEM_1_1, ID_ITEM_1_4, ID_ITEM_1_1, MF_BYCOMMAND);
	return 0;	// 処理したので0.
...
int CMainWindow::OnItem1_2(WPARAM wParam, LPARAM lParam){
	...
	// Item1-2にラジオチェックを付ける.
	CMenu *pSubMenu0 = m_pMainMenu->GetSubMenu(0);
	pSubMenu0->CheckMenuRadioItem(ID_ITEM_1_1, ID_ITEM_1_4, ID_ITEM_1_2, MF_BYCOMMAND);
	return 0;	// 処理したので0.
...
int CMainWindow::OnItem1_3(WPARAM wParam, LPARAM lParam){
	...
	// Item1-3にラジオチェックを付ける.
	CMenu *pSubMenu0 = m_pMainMenu->GetSubMenu(0);
	pSubMenu0->CheckMenuRadioItem(ID_ITEM_1_1, ID_ITEM_1_4, ID_ITEM_1_3, MF_BYCOMMAND);
	return 0;	// 処理したので0.
...
int CMainWindow::OnItem1_4(WPARAM wParam, LPARAM lParam){
	...
	// Item1-4にラジオチェックを付ける.
	CMenu *pSubMenu0 = m_pMainMenu->GetSubMenu(0);
	pSubMenu0->CheckMenuRadioItem(ID_ITEM_1_1, ID_ITEM_1_4, ID_ITEM_1_4, MF_BYCOMMAND);
	return 0;	// 処理したので0.
...
int CMainWindow::OnItem2_2(WPARAM wParam, LPARAM lParam){
	...
	if (ret == IDOK){
		...
		m_pTextFile->SetTextToUtf16LEBytes(tstrText);	// m_pTextFileにセットし, UTF-16LEバイト列に変換し, バッファにセット.
		m_pTextFile->CBinaryFile::Write(dlg.GetOFN().lpstrFile);	// dlg.GetOFN().lpstrFileに書き込み.
	}

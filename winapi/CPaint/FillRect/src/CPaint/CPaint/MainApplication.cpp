...
BOOL CMainApplication::InitInstance(HINSTANCE hInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウの作成.
	if (!pMainWnd->CreateEx(0, _T("CPaint"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance)){	// m_pMainWnd->CreateExでウィンドウ作成し, 失敗した場合.
...
void CMainApplication::Render(){
	...
	CPaint::FillRect(m_hBackBufferDC, rcSquare, RGB(255, 0, 0));	// CPaint::FillRectで(m_x, m_y)位置に赤い四角形を描画.(ブラシの作成・破棄はCPaint側が行うため, Main側でHBRUSHを管理する必要がなくなった.)

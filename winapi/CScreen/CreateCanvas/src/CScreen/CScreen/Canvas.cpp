...
void CCanvas::OnLButtonDown(UINT nFlags, POINT pt){
	...
	// 押した部分に白をセット.
	if (m_pDIBSection != NULL){
		m_pDIBSection->m_lpBitsPixel[(m_pDIBSection->m_iWidth * 3 + (m_pDIBSection->m_iWidth * 3) % 4) * (m_pDIBSection->m_iHeight - (pt.y + 1)) + pt.x * 3] = 0xff;
		m_pDIBSection->m_lpBitsPixel[(m_pDIBSection->m_iWidth * 3 + (m_pDIBSection->m_iWidth * 3) % 4) * (m_pDIBSection->m_iHeight - (pt.y + 1)) + pt.x * 3 + 1] = 0xff;
		m_pDIBSection->m_lpBitsPixel[(m_pDIBSection->m_iWidth * 3 + (m_pDIBSection->m_iWidth * 3) % 4) * (m_pDIBSection->m_iHeight - (pt.y + 1)) + pt.x * 3 + 2] = 0xff;

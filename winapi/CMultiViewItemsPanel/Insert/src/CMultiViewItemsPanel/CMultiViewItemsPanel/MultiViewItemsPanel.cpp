...
void CMultiViewItemsPanel::Remove() {
	...
	if (n > 0) {	// 現在のサイズより大きい時.
		CMultiViewItem* pMultiViewItem = m_vecMultiViewItemList[n - 1];
		m_mapMultiViewItemMap.erase(pMultiViewItem->m_tstrKey);	// 先にマップから消す.
		pMultiViewItem->Destroy();	// 内部を破棄.
		delete pMultiViewItem;	// ポインタを破棄.
		m_vecMultiViewItemList.pop_back();	// リストの末尾から取り出す.
	}

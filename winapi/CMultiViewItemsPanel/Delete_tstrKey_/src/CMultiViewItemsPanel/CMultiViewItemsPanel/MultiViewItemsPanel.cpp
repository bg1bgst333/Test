...
// アイテムを削除する関数Delete.(キー指定バージョン.)
BOOL CMultiViewItemsPanel::Delete(const tstring& tstrKey) {

	// マップから検索.(キーの親はアイテム自身が持つm_tstrKey.)
	std::map<tstring, CMultiViewItem*>::iterator itor = m_mapMultiViewItemMap.find(tstrKey);
	if (itor == m_mapMultiViewItemMap.end()) {
		return FALSE;	// 見つからなければFALSE.
	}

	// 見つかったアイテムのポインタ.
	CMultiViewItem* pMultiViewItem = itor->second;

	// vector内でのインデックスを線形探索.
	size_t n = m_vecMultiViewItemList.size();
	for (size_t ui = 0; ui < n; ui++) {
		if (m_vecMultiViewItemList[ui] == pMultiViewItem) {
			return Delete((int)ui);	// 見つかったインデックスでDelete(int)に委譲.
		}
	}

	// 万が一見つからなければFALSE.(通常は起こらない.)
	return FALSE;
...
// アイテムを取得する関数Get.(キー指定バージョン.)
CMultiViewItem* CMultiViewItemsPanel::Get(const tstring& tstrKey) {

	// マップから検索.(キーの親はアイテム自身が持つm_tstrKey.)
	std::map<tstring, CMultiViewItem*>::iterator itor = m_mapMultiViewItemMap.find(tstrKey);
	if (itor != m_mapMultiViewItemMap.end()) {
		return itor->second;	// 見つかったアイテムを返す.
	}
	else {

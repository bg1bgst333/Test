...
int CMainWindow::OnCreate(HWND hwnd, LPCREATESTRUCT lpCreateStruct){
	...
	// アイテムの追加.
	m_pWindowListView->m_pMultiViewItemsPanel->Add(_T("Item1"), _T("Item1"), 50, 50, 100, 100, m_hInstance);
	m_pWindowListView->m_pMultiViewItemsPanel->Add(_T("Item3"), _T("Item3"), 270, 50, 100, 100, m_hInstance);
	m_pWindowListView->m_pMultiViewItemsPanel->Add(_T("Item2"), _T("Item2"), 160, 50, 100, 100, m_hInstance);
	// アイテムの順番をデバッグ確認.
	OutputDebugString(m_pWindowListView->m_pMultiViewItemsPanel->Get(0)->m_tstrKey.c_str());
	OutputDebugString(_T("\n"));
	OutputDebugString(m_pWindowListView->m_pMultiViewItemsPanel->Get(1)->m_tstrKey.c_str());
	OutputDebugString(_T("\n"));
	OutputDebugString(m_pWindowListView->m_pMultiViewItemsPanel->Get(2)->m_tstrKey.c_str());
	OutputDebugString(_T("\n"));
	// アイテムをキー指定で取得できることを確認.(Item2は挿入順は3番目(インデックス2)だがキーで正しく引けるか確認.)
	CMultiViewItem* pItemByKey = m_pWindowListView->m_pMultiViewItemsPanel->Get(_T("Item2"));
	if (pItemByKey != NULL){
		OutputDebugString(_T("Get(key=Item2) -> "));	// キー取得のラベルを出力.
		OutputDebugString(pItemByKey->m_tstrKey.c_str());	// "Item2"が出力されれば成功.
		OutputDebugString(_T("\n"));
	}
	// アイテムをキー指定で削除.(Item3は挿入順2番目(インデックス1)、途中の要素をキーで削除できるか確認.)
	m_pWindowListView->m_pMultiViewItemsPanel->Delete(_T("Item3"));	// Delete(key)で"Item3"を削除.
	// アイテムの順番をデバッグ確認.(Item3が消え, Item1, Item2の順になっているはず.)
	OutputDebugString(m_pWindowListView->m_pMultiViewItemsPanel->Get(0)->m_tstrKey.c_str());
	OutputDebugString(_T("\n"));
	OutputDebugString(m_pWindowListView->m_pMultiViewItemsPanel->Get(1)->m_tstrKey.c_str());
	OutputDebugString(_T("\n"));

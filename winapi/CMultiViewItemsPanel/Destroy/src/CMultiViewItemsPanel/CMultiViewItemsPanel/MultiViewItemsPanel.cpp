...
BOOL CMultiViewItemsPanel::Destroy(){
	...
	// マルチビューアイテムの破棄.
	if (m_pMultiViewItem != NULL){
		bRet = m_pMultiViewItem->Destroy();
		delete m_pMultiViewItem;
		m_pMultiViewItem = NULL;
	}
...
int CMultiViewItemsPanel::OnCreate(HWND hwnd, LPCREATESTRUCT lpCreateStruct){
	...
	m_pMultiViewItem = new CMultiViewItem();

	m_pMultiViewItem->Create(_T(""), 0, 20, 20, 160, 120, hwnd, (HMENU)IDC_MULTIVIEWITEM, lpCreateStruct->hInstance);

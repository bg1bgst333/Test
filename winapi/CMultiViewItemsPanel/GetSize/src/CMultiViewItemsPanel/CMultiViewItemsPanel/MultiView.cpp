...
int CMultiView::OnCreate(HWND hwnd, LPCREATESTRUCT lpCreateStruct){
	...
	// マルチビューアイテムを3個追加.
	m_pMultiViewItemsPanel->Add(_T("Item1"), 20, 20, 160, 120, lpCreateStruct->hInstance);
	m_pMultiViewItemsPanel->Add(_T("Item2"), 200, 20, 160, 120, lpCreateStruct->hInstance);
	m_pMultiViewItemsPanel->Add(_T("Item3"), 380, 20, 160, 120, lpCreateStruct->hInstance);

	// サイズをデバッグ表示.
	TCHAR tszSize[32] = {0};
	size_t uiSize = m_pMultiViewItemsPanel->GetSize();
	_stprintf(tszSize, _T("uiSize = %lu\n"), uiSize);
	OutputDebugString(tszSize);

	// 1個末尾から削除.
	m_pMultiViewItemsPanel->Remove();

	// もう1度サイズをデバッグ表示.
	ZeroMemory(tszSize, sizeof(TCHAR) * 32);
	uiSize = m_pMultiViewItemsPanel->GetSize();
	_stprintf(tszSize, _T("uiSize = %lu\n"), uiSize);
	OutputDebugString(tszSize);

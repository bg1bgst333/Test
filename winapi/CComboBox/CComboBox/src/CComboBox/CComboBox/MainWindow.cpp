...
int CMainWindow::OnCreate(HWND hwnd, LPCREATESTRUCT lpCreateStruct){
	...
	if (m_pMainMenu == NULL){	// メニューハンドルが無い場合は, m_pMainMenuがNULLになる.
		...
		if (bRet){
			...
			pComboBox1->Create(_T("ComboBox"), _T("Item0-ComboBox1"), WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST, 0, 0, rc.right - rc.left, 25, pItem0->m_hWnd, (HMENU)WM_APP + 200, m_hInstance);

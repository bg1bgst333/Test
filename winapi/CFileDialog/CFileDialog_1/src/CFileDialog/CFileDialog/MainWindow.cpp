...
int CMainWindow::OnItem2_1(WPARAM wParam, LPARAM lParam) {
	...
	if (ret == IDOK) {
		MessageBox(m_hWnd, _T("IDOK"), _T("CFileDialog"), MB_OK);
		MessageBox(m_hWnd, dlg.GetOFN().lpstrFile, _T("CFileDialog"), MB_OK);
	}
	else {
		MessageBox(m_hWnd, _T("IDCANCEL"), _T("CFileDialog"), MB_OK);
	}

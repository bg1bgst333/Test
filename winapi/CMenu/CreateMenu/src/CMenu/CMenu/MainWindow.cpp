...
int CMainWindow::OnCreate(HWND hwnd, LPCREATESTRUCT lpCreateStruct) {
	...
	if (m_pMainMenu == NULL) {	// メニューハンドルが無い場合は, m_pMainMenuがNULLになる.
		...
		if (bRet) {
			m_pMainMenu->AppendMenu(MF_STRING, 101, _T("Item1"));	// Item1追加.
			m_pMainMenu->AppendMenu(MF_STRING, 102, _T("Item2"));	// Item2追加.
			m_pMainMenu->AppendMenu(MF_STRING, 103, _T("Item3"));	// Item3追加.

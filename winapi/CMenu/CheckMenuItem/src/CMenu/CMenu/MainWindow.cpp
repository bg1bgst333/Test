...
int CMainWindow::OnItem1_1(WPARAM wParam, LPARAM lParam){
	...
	// Item1-1にチェック付けたり外したりする.
	CMenu *pSubMenu0 = m_pMainMenu->GetSubMenu(0);
	UINT uiState = pSubMenu0->GetMenuState(ID_ITEM_1_1, MF_BYCOMMAND);
	if (uiState & MF_CHECKED){
		pSubMenu0->CheckMenuItem(ID_ITEM_1_1, MF_UNCHECKED | MF_BYCOMMAND);
	}
	else{
		pSubMenu0->CheckMenuItem(ID_ITEM_1_1, MF_CHECKED | MF_BYCOMMAND);
	}
	return 0;	// 処理したので0.

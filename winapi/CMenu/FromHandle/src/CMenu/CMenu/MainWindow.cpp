...
int CMainWindow::OnCreate(HWND hwnd, LPCREATESTRUCT lpCreateStruct) {
	...
	HMENU hMainMenu = GetMenu(hwnd);	// メインメニューを取得.
	if (hMainMenu != NULL) {	// あった場合.
		m_pMainMenu = CMenu::FromHandle(hMainMenu);	// メニューオブジェクト取得.
		HMENU hSubMenu0 = m_pMainMenu->GetSubMenuHandle(0);	// 0番目のサブメニュー取得.
		if (hSubMenu0 != NULL) {	// あった場合.
			CMenu* pSubMenu0 = CMenu::FromHandle(hSubMenu0);	// メニューオブジェクト取得.
...
void CMainWindow::OnDestroy() {
	...
	// メニューハンドルマップの全削除.
	std::map<HMENU, CMenu*>::iterator itor = CMenu::m_mapMenuHandleMap.begin();
	while (itor != CMenu::m_mapMenuHandleMap.end()) {
		HMENU hMenu = itor->first;
		CMenu* pMenu = itor->second;
		DestroyMenu(hMenu);
		if (pMenu == m_pMainMenu) {
			m_pMainMenu = NULL;
		}
		delete pMenu;
		itor++;
	}
	CMenu::m_mapMenuHandleMap.clear();

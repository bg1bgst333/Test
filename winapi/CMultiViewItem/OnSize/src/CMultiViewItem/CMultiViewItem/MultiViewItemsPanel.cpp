...
void CMultiViewItemsPanel::OnSizeChild(WPARAM wParam, LPARAM lParam) {
	...
	// "CMultiViewItemsPanel::OnSizeChild"とデバッグ出力.
	OutputDebugString(_T("CMultiViewItemsPanel::OnSizeChild\n"));

	// サイズを取り出し, 出力.
	int iWidth = LOWORD(wParam);
	int iHeight = HIWORD(wParam);
	TCHAR tszWH[256] = { 0 };
	_stprintf(tszWH, _T("iWidth = %d, iHeight = %d\n"), iWidth, iHeight);
	OutputDebugString(tszWH);

...
int CMainWindow::OnCreate(HWND hwnd, LPCREATESTRUCT lpCreateStruct) {
	...
	if (m_pMainMenu == NULL) {	// メニューハンドルが無い場合は, m_pMainMenuがNULLになる.
		...
		if (bRet) {
			...
			pCustomEdit->Create(_T("Item0-CustomEdit"), WS_CHILD | WS_VISIBLE | WS_HSCROLL | WS_VSCROLL | ES_MULTILINE | ES_WANTRETURN | ES_AUTOHSCROLL | ES_AUTOVSCROLL, 10, 10, 160, 80, pItem0->m_hWnd, (HMENU)WM_APP + 200, m_hInstance);
		}
	}
	return iRet;	// iRetを返す.

...
void CMainApplication::Input(){
	...
	// 矢印キーの押下状態を調べて, m_Characterの速度(m_vx, m_vy)に反映する.(座標の更新自体はLogicで行う.)
	if (CKeyboardInput::IsKeyDown(VK_RIGHT)){	// 右矢印キーが押されている場合.
		m_Character.MoveRight();	// m_Character.MoveRightで右へ移動させる.
	}else if (CKeyboardInput::IsKeyDown(VK_LEFT)){	// 左矢印キーが押されている場合.
		m_Character.MoveLeft();	// m_Character.MoveLeftで左へ移動させる.
	}else{	// どちらも押されていない場合.
		m_Character.StopHorizontal();	// m_Character.StopHorizontalで水平方向の速度を止める.
	}
	if (CKeyboardInput::IsKeyDown(VK_DOWN)){	// 下矢印キーが押されている場合.
		m_Character.MoveDown();	// m_Character.MoveDownで下へ移動させる.
	}else if (CKeyboardInput::IsKeyDown(VK_UP)){	// 上矢印キーが押されている場合.
		m_Character.MoveUp();	// m_Character.MoveUpで上へ移動させる.
	}else{	// どちらも押されていない場合.
		m_Character.StopVertical();	// m_Character.StopVerticalで垂直方向の速度を止める.
	}
...
void CMainApplication::Logic(){
	...
	// m_Characterの速度(m_vx, m_vy)の分だけm_Characterの座標を進める.
	m_Character.m_x += m_Character.m_vx;	// m_Character.m_xにm_Character.m_vxを加算.
	m_Character.m_y += m_Character.m_vy;	// m_Character.m_yにm_Character.m_vyを加算.

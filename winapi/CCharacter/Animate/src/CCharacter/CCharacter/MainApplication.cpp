...
BOOL CMainApplication::InitInstance(HINSTANCE hInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// キャラクターのスプライト画像のロード.
	if (!m_Character.Load(hInstance)){	// CCharacter::Loadで画像をロードし, 失敗した場合.

		// エラー処理
		MessageBox(m_pMainWnd->m_hWnd, _T("CCharacter::Load failed!"), _T("CCharacter"), MB_OK | MB_ICONHAND);	// MessageBoxで"CCharacter::Load failed!"とエラーメッセージを表示.
		return FALSE;	// returnでFALSEを返して異常終了.

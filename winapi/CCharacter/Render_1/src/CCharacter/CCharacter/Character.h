...
class CCharacter{
	...
	public:
		...
		CSprite m_Sprite;	// 自分の見た目を描くCSpriteオブジェクトm_Sprite.(Loadで画像を読み込んでおく. 幅・高さもここから取得する.)

		// publicメンバ関数
		// コンストラクタ
		CCharacter();	// コンストラクタCCharacter
		// メンバ関数
		virtual BOOL Load(HINSTANCE hInstance);	// 画像読込関数Load.(m_Spriteに画像をロードさせる.)
		virtual void Render(HDC hDC);	// 描画処理関数Render.(指定されたHDCに, 自分の位置(m_x, m_y)にm_Spriteを描画する.)

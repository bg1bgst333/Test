...
BOOL CCharacter::Load(HINSTANCE hInstance){
	...
	// m_SpriteにLoadさせる.
	return m_Sprite.Load(hInstance, _T("walk1.bmp"));	// m_Sprite.Loadの戻り値をそのまま返す.
...
void CCharacter::Render(HDC hDC){
	...
	// m_Spriteの描画位置を自分の座標に合わせる.
	m_Sprite.m_x = m_x;	// m_Sprite.m_xにm_xを反映.
	m_Sprite.m_y = m_y;	// m_Sprite.m_yにm_yを反映.

	// m_Sprite.Drawで自分を描画する.
	m_Sprite.Draw(hDC);	// m_Sprite.Drawでスプライト画像を描画.
...
void CCharacter::Logic(){
	...
	// 画面端で止まるようにクランプする.(跳ね返らず, それ以上は進めない. サイズはm_Spriteの実寸を使う.)
	if (m_x < 0){	// 左端を超えた場合.
		m_x = 0;	// 左端に合わせる.
	}
	if (m_x > m_pGraphicalApplication->m_iBackBufferWidth - m_Sprite.m_iWidth){	// 右端を超えた場合.
		m_x = m_pGraphicalApplication->m_iBackBufferWidth - m_Sprite.m_iWidth;	// 右端に合わせる.
	}
	if (m_y < 0){	// 上端を超えた場合.
		m_y = 0;	// 上端に合わせる.
	}
	if (m_y > m_pGraphicalApplication->m_iBackBufferHeight - m_Sprite.m_iHeight){	// 下端を超えた場合.
		m_y = m_pGraphicalApplication->m_iBackBufferHeight - m_Sprite.m_iHeight;	// 下端に合わせる.
	}
